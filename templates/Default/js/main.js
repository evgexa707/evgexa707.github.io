...
$(document).ready(function(){
	var mw = ($('html, body').width()-802)/2;
	var mw2 = ($('html, body').width()-1152)/2;
	if($('.autowr').css('padding-left', mw+'px').css('padding-right', mw2+'px')){
		$('body').show();
...
function onBodyResize(){
	var mw = ($('html, body').width()-802)/2;
	var mw2 = ($('html, body').width()-1152)/2;
	$('.autowr').css('padding-left', mw+'px').css('padding-right', mw2+'px');
}
var Page = {
	Loading: function(f){
	...
	Go: function(h){
		...
		if($('#aid').val() || $('#back').val()){
			$('#backbtn').removeClass('opacity0');
		} else {
			$('#backbtn').addClass('opacity0');
		}			
...
var Profile = {
	...
	LoadCity: function(id){
		...
		if(id > 0){
			$('#city').slideDown();
			$('#select_city1').load('/index.php?go=loadcity', {country: id});
			$('.resetvalue').val('- Не выбран -');
		} else {
			$('#city').slideUp();
			$('#load_mini').hide();
		}
	...
	LoadPhoto: function(){
		...
		$.get('/index.php?go=editprofile&act=load_photo', function(data){
			Box.Show('photo', 500, lang_title_load_photo, data, lang_box_canсel);
			Page.Loading('stop');
		});
	},
	DelPhoto: function(photo_id){
		Box.Show('del_photo', 400, lang_title_del_photo, '<div style="padding:15px;">'+lang_del_photo+'</div>', lang_box_canсel, lang_box_yes, 'Profile.StartDelPhoto('+photo_id+'); return false;');
	},
	StartDelPhoto: function(photo_id){
		$('#box_loading').show();
		$.get('/index.php?go=editprofile&act=del_photo', {photo_id: photo_id}, function(){
			Page.Go('/');
			$('#del_pho_but').hide();
			Box.Close('del_photo');
			Page.Loading('stop');
...
var viiBox = {
	...
	win: function(i, d, o, h){
		...
		$('html, body').css('overflow-y', 'hidden');
		if(is_moz && !is_chrome) $(window).scrollTop(scrollTopForFirefox);
		$('body').append('<div class="vii_box no_display" id="newbox_miniature'+i+'">'+d+'</div>');
		$('.vii_box').fadeIn(200);
		$(window).keydown(function(event){
			if(event.keyCode == 27) 
				viiBox.clos(i, o, h);
	...
	clos: function(i, o, h){
		...
		if(o) $('html, body').css('overflow-y', 'visible');
...
var Box = {
	...
	Show: function(name, width, title, content, close_text, func_text, func, height, overflow, bg_show, bg_show_bottom, cache){
		...
		if(overflow)
			var overflow = 'overflow-y:visible;';
		else
			var overflow = '';
