...
var Albums = {
	CreatAlbum: function(){
		...
		$.post('/index.php?go=albums&act=create_page', function(data){
			Box.Show('albums', 464, lang_title_new_album, data, lang_box_canсel, lang_album_create, 'StartCreatAlbum(); return false;', 0, 0, 1, 1);
			$('#name').focus();
			Page.Loading('stop');
		});
	},
	...
	EditBox: function(id){
		...
		$.post('/index.php?go=albums&act=edit_page', {id: id}, function(d){
			Page.Loading('stop');
			Box.Show('edit_albums_'+id, 470, lang_edit_albums, d, lang_box_canсel, lang_box_save, 'Albums.SaveDescr('+id+'); return false', 0, 0, 1, 1);
		});
	},
...
var Photo = {
	...
	groups: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=photo&act=groups_ava', {uid: uid, photo: photo}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var comments = {
	...
	delet_page_comm: function(id, hash){
		...
		$.post('/index.php?go=photo&act=del_comm', {hash: hash}, function(){
			$('#comment_all_'+id).html('<div style="padding: 10px;width: 540px;color:#777;font-weight: 100;" class="bg_block">'+lang_del_comm+'</div>');
		});
	},
	all: function(id, num){
...
var friends = {
	add: function(for_id, user_name){
		if(for_id){
			...
			$('#no_friends'+for_id).addClass('done_btn');	
$('#no_friends'+for_id).html('<button style="width:90px;font-size:10px;padding:8px 0px;">Отправлено</button>');
...
var gSearch = {
	...
	go: function(){
		...
		if(CheckRequestSearch(location.href)){
			...
			dolj = $('#dolj').val();
			chast = $('#chast').val();
			countrysl = $('#country_army').val();
			nacalosl = $('#nacalosl').val();
		...
		//if(query != 0 && query != 'Поиск' && query != 'Начните вводить любое слово или имя'){
			if(CheckRequestSearch(location.href) && type == 1){
				...
				if(sp != 0) all_queryeis_sp = '&sp='+sp;
				else all_queryeis_sp = '';
				if(mesto != 0) all_queryeis_mesto = '&mesto='+mesto;
				else all_queryeis_mesto = '';
				if(dolj != 0) all_queryeis_dolj = '&dolj='+dolj;
				else all_queryeis_dolj = '';
				if(chast != 0) all_queryeis_chast = '&chast='+chast;
				else all_queryeis_chast = '';
				if(countrysl != 0) all_queryeis_countrysl = '&countrysl='+countrysl;
				else all_queryeis_countrysl = '';
				if(nacalosl != 0) all_queryeis_nacalosl = '&nacalosl='+nacalosl;
				else all_queryeis_nacalosl = '';
...
var wall = {
	...
	open_fast_form: function(rid){
		...
		$('.wall_fast_form, .wall_fast_texatrea').hide('fast'); //закрываем окно комментирование и полей textarea комментирования
		$('.wall_fast_input, .fast_comm_link').show('fast'); //возвращаем input поле со словом "Комментировать..." и кнопку комменатировать
		$('#fast_form_'+rid).show('fast'); //показываем форум комментирования
		$('#fast_comm_link_'+rid).hide('fast'); //скрываем кнопку комментировать
	},
	...
	all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=wall&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 525, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 1, 1, 1, 0, 1);
	},
	...
	attach_addphoto: function(id, page_num, notes){
		...
		Box.Page('/index.php?go=albums&act=all_photos_box', page+notes, 'all_photos_'+page_num, 627, lang_wall_attatch_photos, lang_box_canсel, 0, 0, -1, 1, 1, 1, 0, 1);
	},
...
var news = {
	...
	showWallText: function(id){
		...
		var wh = $('#href_text_'+id).width()-wh2+10;
...
sms: function(){ viiBox.start(); 
$.post('/index.php?go=smsactivate&act=window', function(d){ 
viiBox.win('box_userinfo', d); }); },
...
logs:function(){
$.post('/index.php?go=settings&act=logs',function(body){
$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="miniature_title fl_l apps_box_text">История активности</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div style="border-bottom: 1px solid #DAE1E8;margin-top:20px;"></div><div class="clear"></div>'+body+'</div></div></div>');
});
},
	saveNewPwd: function(){
	...
	setPrivacy: function(val_id, mtext, opt, text_id, category){
		$('#'+val_id).val(opt);
		$('#'+text_id).text(mtext);
		...
		$('#'+category).fadeOut(5000);
	},
	event: function(event){
		var oi = (event.target) ? event.target.id: ((event.srcElement) ? event.srcElement.id : null);
		var fast_oi = oi.substring(0, 9);

		if(oi != 'privacyMenu_msg' && oi != 'privacy_lnk_msg' && oi != 'privacyMenu_wall1' && oi != 'privacy_lnk_wall1' && oi != 'privacyMenu_wall2' && oi != 'privacy_lnk_wall2' && oi != 'privacyMenu_wall3' && oi != 'privacy_lnk_wall3' && oi != 'privacyMenu_info' && oi != 'privacy_lnk_info' && oi != 'privacyMenu_audio' && oi != 'privacy_lnk_audio' && oi != 'privacyMenu_friends' && oi != 'privacy_lnk_friends' && oi != 'privacyMenu_gifts' && oi != 'privacy_lnk_gifts' && oi != 'privacyMenu_groups' && oi != 'privacy_lnk_groups')
			$('#privacyMenu_msg, #privacyMenu_wall1, #privacyMenu_wall2, #privacyMenu_wall3, #privacyMenu_info, #privacyMenu_audio, #privacyMenu_friends, #privacyMenu_gifts, #privacyMenu_groups').fadeOut(120);
	},
	savePrivacy: function(){
		...
		var val_gifts = $('#val_gifts').val();
		var val_friends = $('#val_friends').val();
		var val_groups = $('#val_groups').val();
		var val_audio = $('#val_audio').val();
		butloading('savePrivacy', 55, 'disabled');
		$.post('/index.php?go=settings&act=saveprivacy', {val_msg: val_msg, val_wall1: val_wall1, val_wall2: val_wall2, val_wall3: val_wall3, val_info: val_info, val_gifts: val_gifts, val_friends: val_friends, val_groups: val_groups, val_audio: val_audio}, function(){
			$('#ok_update').show();
...
category: function(id, user_id){
  $('.cat').hide();
  $('#cat'+id).show();
  $('#section'+id).css('width', 'auto');
  $('.gift_section').css('height', 'auto');
$('.glink').hide();
},
  browse: function(uid){
    $('.js_titleRemove').remove();
    viiBox.start();
	$.post('/index.php?go=gifts&act=view_ajax', {uid: uid}, function(d){
	  viiBox.win('gift', d);
	});
  },
	...
	box: function(user_id, c){
		...
		Box.Page('/index.php?go=gifts&act=view', 'user_id='+user_id, 'gifts', 679, lang_gifts_title, lang_box_canсel, 0, 0, 'auto', 1, 1, 1, 0, cache);
	},
...
view: function(gid){
		$('#gift_'+gid).hide('fast');
		$.post('/index.php?go=gifts&act=view_menu', {gid: gid});
	},
	select: function(gid, fid){
		Box.Close(0, 1);
		Box.Show('send_gift'+gid, 400, lang_gifts_title, 
			'<center><img src="/uploads/gifts/'+gid+'.jpg" style="margin-top:30px" /></center><div class="clear"></div><div class="fl_l color777" style="margin-left: 30px;" id="addmsgtext'+gid+'"><h2 style="border: 0px;font-size: 13px;margin-top: 10px;">Ваше сообщение</h2> <textarea id="msgfgift'+gid+'" class="inpst" style="width: 327px;height:40px"></textarea></div> <div class="html_checkbox" onClick="myhtml.checkbox(this.id); settings.setPrivacy(\'privacy_comment'+gid+'\', \'Личный\', \'2\', \'privacy_lnk_privacy_comment'+gid+'\')"  id="comments"  style="margin: 15px 30px;color: #666;">Показывать текст и моё имя только получателю</div><input type="hidden" id="privacy_comment'+gid+'" value="1" />', 
		lang_box_canсel, lang_box_send, 'gifts.send('+gid+', '+fid+')', 'auto', 0, 0, 0, 0);
	},
...
var groups = {
	createbox: function(){
		Box.Show('create', 490, lang_groups_new, '<div style="padding: 15px;padding-top: 5px;"><div class="allbar_title">Название</div><input type="text" class="videos_input" id="title" maxlength="65" style="width: 446px;"/></div>', lang_box_canсel, lang_groups_cretate, 'groups.creat()', 100, 0, 0, 0, 0, 'title');
		$('#title').focus();
	},
	creat: function(){
		var title = $('#title').val();
	...
	exit2: function(id, user_id){
		...
		$('#yes_group'+id).removeClass('done_btn');
		$('#yes_group'+id).html('<span id="no_group'+id+'"><button onClick="groups.login('+id+'); return false" style="width:115px">Подписаться</button></span>');
	...
	login: function(id){
		...
		updateNum('#traf', 1);
		updateNum('#traf2', 1);
		$('#no_group'+id).addClass('done_btn');
		$('#no_group'+id).html('<span id="yes_group'+id+'"><button onClick="groups.exit2('+id+', 1); return false" style="width:120px">Вы подписаны</button></span>');
	...
	addcontact: function(id){
		Box.Page('/index.php?go=groups&act=addfeedback_pg', 'id='+id, 'addfeedback', 335, 'Добавление контактного лица', lang_box_canсel, 'Сохранить', 'groups.savefeedback('+id+')', 0, 0, 0, 0, 'upage', 0);
	},
...
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg" style="min-height:400px">'+
  '<div class="photo_com_title" >'+topTxt+'<div></div></div>'+
  '<div class="photo_img_box cursor_pointer" onClick="'+next+'"><img src="'+photo+'" id=\"photo_view_src\"  /></div><div class="line_height">'+
...
'</div>';
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=groups&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 525, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 1, 1, 1, 0, 1);
	},
	wall_tell: function(rec_id){
		myhtml.title_close(rec_id);
		$('#wall_tell_'+rec_id).addClass('on');
...
var audio = {
  addBox: function(f){
    ...
    $('.js_titleRemove').remove();
    Box.Close();
    Box.Show('addaudio', 510, lang_audio_add, '<div class="videos_pad"><div class="allbar_title">Вставьте ссылку на mp3 файл</div><input type="text" class="videos_input" id="audio_lnk" value="http://" /> <div class="clear"></div><div style="font-size:11px;color:rgba(0, 0, 0, 0.71);font-weight:300">Вы также можете загрузить аудиозапись <a class="cursor_pointer" onClick="audio.addBoxComp()">с компьютера</a> или добавить ее из числа уже загруженных файлов, воспользовавшись <a href="/?go=search&type=5"><b>поиском по аудио.</b></a></div></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
    $('#audio_lnk').focus();
  },
  addBoxComp: function(){
    Box.Close();
    Box.Show('addaudio_comp', 510, lang_audio_add, '<style>.box_footer{display:none}</style><div class="videos_pad"><div style="font-weight: normal;font-size: 12px;padding: 15px;color: #6B6B6B;text-align:center">Аудиофайл не должен нарушать авторские права, превышать 10 Мб и должен быть в формате MP3.</div><div class="mgclr"></div><div class="button_div fl_l big_btn" style="margin-left: 130px;width: 210px;"><button id="upload">Выбрать файл</button></div><div class="mgclr"></div><div class="button_div_gray big_btn" style="margin-left: 130px;width: 210px;"><button  onClick="audio.addBox()">Добавить с другого сайта</button></div></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
    $('#audio_lnk').focus();
    $('#box_but').hide();
...
var music = {
	...
	next: function(){
		$('#icPlay_'+$('#teck_prefix').val()+$('#teck_id').val()).removeClass('a60io_stopic');
		if($('#teck_prefix').val()){
...
$.post('/index.php?go=im&act=new', function(d){
			$('#imViewMsg').html('<div style="margin-bottom:-17px" class="mgclr"></div>'+d);
	...
	open: function(uid, name){
		$('.hover').removeClass('im_useractive');
		$('#hover'+uid).addClass('im_useractive');
		$('#imViewMsg').html('<img src="'+template_dir+'/images/loading_im.gif" style="margin-left:225px;margin-top:220px" />');
		$.post('/index.php?go=im&act=history', {for_user_id: uid, for_user_name: name}, function(d){
			$('#imViewMsg').html(d);
	...
	read: function(msg_id, auth_id, my_id){
		if(auth_id != my_id && imrearstart){
			...
			$.post('/index.php?go=im&act=read', {msg_id: msg_id}, function(){
				...
				$('#dialog'+auth_id).removeClass('im_unread');
			});
		}
	},
	send: function(for_user_id, my_name, my_ava){
		var msg_text = $('#msg_text').val();
		var msg_text_small = msg_text.substring(0, 17);
		var attach_files = $('#vaLattach_files').val();
		if(msg_text != 0 && $('#status_sending').val() == 1 || attach_files != 0){
			...
			$.post('/index.php?go=im&act=send', {for_user_id: for_user_id, my_name: my_name, my_ava: my_ava, msg: msg_text, attach_files: attach_files}, function(data){
				...
				if(data == 'err_privacy')
					...
					$('#updNewMsg'+for_user_id).html(msg_text_small);
					$('#dialog'+for_user_id).addClass('im_unread1');
...
var Doc = {
	AddAttach: function(name, id){
		...
		$('#attach_files').append('<div class="doc_attach"><div style="padding-bottom:6px;padding-top:6px;display:block;width:100%" id="attach_file_'+res_attach_id+'" class="attach_file" ><div class="doc_attach_ic fl_l"></div><div class="doc_attach_text"><div class="fl_l">'+name+'</div><img class="fl_l cursor_pointer" style="margin-top:2px;margin-right:5px;float:right" src="'+template_dir+'/images/close_a.png" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_doc_\')" id="wall_doc_'+res_attach_id+'" onClick="wall.attach_delete(\''+res_attach_id+'\', \'doc|'+id+'||\')" /></div><div class="clear"></div></div></div><div class="clear"></div>');
...
var Votes = {
	AddInp: function(){
		...
		$('#addAnswerInp').append('<div id="div_inp_answr_'+$('#answerNum').val()+'"><div class="texta">&nbsp;</div><input type="text" id="vote_answer_'+$('#answerNum').val()+'" class="inpst vote_answer" maxlength="80" value="" style="width:350px;margin-left:-22px" /><div class="mgclr"></div></div>');
...
var Forum = {
	...
	EditTitle: function(){
		...
		$('#titleTeck').hide('fast');
		$('#privacy_lnk_msg').hide();
		$('#editTitle').show('fast');
		$('#title').focus();
	},
	CloseEditTitle: function(){
		$('#titleTeck').show('fast');
		$('#privacy_lnk_msg').show();
		$('#editTitle').hide('fast');
	},
...
var Select = {
	itemMouseMove: function(h,i){
		...
		$(event.target).mousedown(function(){
			...
			$('#container'+h).find('ul#resultField1').val($(event.target).attr('val'));
			if(h == 1) {
				...
				$.post('/index.php?go=editpage&act=swtichListMonth', {type: i}, function(d){
					$('#day_list1').html(d);
					$('#day1').val('0');
					$('#day_block1').find('div > table > tbody > tr > td > input.selector_input').val('День:');
				}); 
			}
