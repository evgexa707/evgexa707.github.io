...
[record]<div class="wallrecord wall_upage" id="wall_record_{rec-id}" style="margin-top:5px">
 <div style="float:left;width:65px"><div class="ava_mini [privacy-comment][if-comments]wall_ava_mini[/if-comments][/privacy-comment]" id="ava_rec_{rec-id}"><a href="/u{user-id}" onClick="Page.Go(this.href); return false"> <img src="{ava}" alt="" title="" /></a><div class="online_ic"></div></div></div>
 ...
 [owner]
<div class="wall_delete" onMouseOver="myhtml.title('{rec-id}', 'Удалить запись', 'wall_del_')" onClick="wall.delet('{rec-id}'); return false" id="wall_del_{rec-id}"></div>[/owner]

 [owner-record]
<div class="wall_tell cursor_pointer" onMouseOver="myhtml.title('{rec-id}', 'Рассказать друзьям', 'wall_tell_')" onClick="wall.tell('{rec-id}'); return false" id="wall_tell_{rec-id}" style="margin-top:2px;margin-left:4px"></div>
 ...
 <div class="infowalltext_f clear">
  <div class="fl_l"><a href="/wall{author-id}_{rec-id}" onClick="Page.Go(this.href); return false" class="online">{date}</a> </div><span id="fast_comm_link_{rec-id}" class="fast_comm_link"><a href="/" id="fast_link_{rec-id}" onclick="wall.open_fast_form('{rec-id}'); wall.fast_open_textarea('{rec-id}'); return false" class="comment_ico" onmouseover="myhtml.title('{rec-id}', 'Комментировать', 'fast_link_')"></a></span>
  ...
  <div class="fl_r public_wall_like cursor_pointer" onClick="{like-js-function}" onMouseOver="groups.wall_like_users_five('{rec-id}', 'uPages')" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" id="wall_like_link{rec-id}">
   ...
   <b id="wall_like_cnt{rec-id}" class="" style="color: rgb(47, 88, 121);">{likes}</b>


  </div>
 <div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{rec-id}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{rec-id}'); return false "id="wall_tell_all_{rec-id}">
