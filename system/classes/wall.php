			...
			if($row_wall['attach']){
				...
				foreach($attach_arr as $attach_file){
					...
					//Видео
						...
						$attach_result .= "<div><a href=\"/video{$attach_type[3]}_{$attach_type[2]}\" onClick=\"videos.show({$attach_type[2]}, this.href, location.href); return false\"><img src=\"/uploads/videos/{$attach_type[3]}/{$attach_type[1]}\" style=\"margin-top:3px;margin-right:3px; width:300px\" align=\"left\" /></a></div>";
...
/*
					...
					//Если документ
                        ...
                        $row_doc = $db->super_query("SELECT dname, dsize, file_type, ddownload_name, duser_id, jpgif, did FROM `vii_doc` WHERE did = '{$doc_id}'");
...
<img src="/uploads/doc/'.$row_doc['duser_id'].'/'.$row_doc['ddownload_name'].'" class="pages_gif_img page_gif_big"></a>
<a onclick="gif.show('.$row_doc['did'].');" class="photo page_doc_photo_href" id="gif_'.$row_doc['did'].'">
<div class="page_doc_photo" style="background-image: url(/uploads/doc/'.$row_doc['duser_id'].'/'.$row_doc['jpgif'].');"></div>
...
$pat = array('/#([A-Za-z0-9А-ЯЁа-яёйцукенгшщзхъфывапролджэячсмитьбю]+)/', '/@(\w+)/');
$rep = array('<a href="/?go=search&query=%23$1&type=7">#$1</a>','<a href="/$1">@$1</a>');
