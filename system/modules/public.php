...
if($logged){
	...
	if($page_cnt){
		...
		$row = $db->super_query("SELECT id, title, descr, traf, ulist, photo, date, admin, verification, feedback, comments, real_admin, rec_num, del, ban, adres, audio_num, forum_num, discussion, status_text, web, videos_num, cover, cover_pos FROM `".PREFIX."_communities` WHERE ".$sql_where."");
	...
	if($row['del'] == 1){
		...
		$verification = '<img onmouseover="myhtml.title(\'verifi\', \'Подтвёржденное сообщество ' . $row['tile'] . ' \', \'\');" id="verifi" style="background:url(\'{theme}/images/verifi.png\');width:13px;height:11px;margin-left:5px;margin-top:-5px" src="{theme}/images/spacer.gif" />';
