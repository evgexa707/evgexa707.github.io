			...
			if($mass_type <= 24 AND $mass_type >= 1){
				foreach($massaction_users as $user_id){
					...
					//Верификацию пользователя
						...
						if($check_user['cnt'])
							$db->query("UPDATE `".PREFIX."_users` SET verification = '1' WHERE user_id = '{$user_id}'");
				...
				//Информация о удаление документов
					...
					msgbox('Верификация', 'Выбраные пользователи были верификацированы', '?mod=users');
				else if($mass_type == 24)
					msgbox('Верификация', 'Выбраные пользователи потеряли верификацированы', '?mod=users');
